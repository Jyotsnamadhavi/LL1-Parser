# -*- coding: utf-8 -*-
"""
Created on Sun Feb  3 03:34:58 2019

@author: sweety
"""

## python Ex2.py test1.txt
...
def LexicalResult(file_data,list_of_tokens,value_of_tokens):
				...
				while(i<len(token)):
					if token[i] in operator_keys :
						if(i+1<len(token)) :
						 ...
						 value_of_tokens.append(ERROR.format(token=token[i]))
						 error_flag = 1
						 break
			if error_flag == 1:
				break
	...
	return list_of_tokens,value_of_tokens
...
"""
Created on Tue Mar  5 15:44:35 2019
...
def LL1(list_of_tokens):
	...
	while (1):
		...
		if(stack[-1]in ColIdxHash):
			if(stack[-1]=="epsilon"):
				stack.pop()
			if(stack[-1]!=list_of_tokens[k] and stack[-1]in ColIdxHash):
...
def printSymboleTable(value_of_tokens,list_of_tokens):
	symbolname=[]
	symboleType=[]
	index=[]
	i=0
	flow=""
	list_of_tokens.pop()
	#print(list_of_tokens,value_of_tokens)
	for indexNum, token, in enumerate(list_of_tokens):
		#print(value_of_tokens[indexNum], indexNum)
		if value_of_tokens[indexNum] not in operator_keys:
			if (value_of_tokens[indexNum]=='BEGIN'):
				flow="begin started"
			if flow:
				if value_of_tokens[indexNum] not in KEYWORDS:
					if list_of_tokens[indexNum]=='ID':
						if value_of_tokens[indexNum] not in symbolname:
							return "Invalid Syntax.."+value_of_tokens[indexNum]+" undefined."
					else:
						index.append(value_of_tokens[indexNum]+","+token)	
						
						
			elif (value_of_tokens[indexNum]=='END'):
				flow=""	
			elif value_of_tokens[indexNum] not in KEYWORDS:
				if value_of_tokens[indexNum] not in symbolname:
					index.append(value_of_tokens[indexNum]+","+token)
					symbolname.append(value_of_tokens[indexNum])
				else:
					return "Invalid Syntax.."+value_of_tokens[indexNum]+" redefined."
...
def main():
	...
	list_of_tokens=[]
	value_of_tokens=[]
	list_of_tokens,value_of_tokens=LexicalResult(file_data,list_of_tokens,value_of_tokens)
	#print(list_of_tokens)
	status=LL1(list_of_tokens)
	#print(status)
	#text_file.close()	   
	count=0;
	if(status=="Code compiled successfully"):
		index=printSymboleTable(value_of_tokens,list_of_tokens)
		if "Invalid Syntax.."in index:
			print(index)
		else:
			print("Program has been parsed successfully")
			print("INDEX"+" "+"SYMBOL NAME"+" "+"SYMBOL TYPE")
			print("--------------------------------")
			for i in index:
				i=i.split(',')
				print(count,"    "+i[0],"        "+i[1])
